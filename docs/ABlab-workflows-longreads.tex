...
\item
  Wash twice with 200 µl 80\% OH in same 96-well plate and let alcohol evaporate for \textasciitilde{} 5 mins or until alcohol is completely removed.
\item
...
\tightlist
\item
  Follow the protocol \textbf{Native Barcoding Kit 96 V14 (SQK-NBD114.96)} from Oxford Nanopore.
\item
...
\item
  Print out and follow the Kit's protocol and checklist (\texttt{Ligation\ sequencing\ amplicons\ -\ Native\ Barcoding\ Kit\ 96\ V14\ (SQK-NBD114.96)-minion.pdf}).
\item
  ...
  \tightlist
  \item
    To achieve consistent (normalised) concentrations (200 fmol) across all samples, add appropriate input volume (calculated with C1\emph{V1 = C2}V2) and fill to 11.5 µl with H20 to the first step of the Native Barcoding kit protocol (`Ligation sequencing amplicons - Native Barcoding Kit 96 V14 (SQK-NBD114.96)-minion.pdf'.
  \item
...
\subsection{Outline of steps}\label{outline-of-steps}
