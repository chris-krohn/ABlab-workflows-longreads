...
![Volumes to normalise to 200 fmol (Example). ](./img/PCR_minDNA.png){width="400"}

\

![Example of a worksheet for calculating volumes (µl) of PCR products and H~2~0 to get 200 fmol in 11.5 µl, required as input for each sample with the Native Barcoding Kit. C1 = DNA concentration (ng/µl), C2 (library concentration) = 45.22 ng/µl, V2 (H~2~0 volume) = 11.5 µl.](./img/PCR_C1V1.png){width="300"}
...
2.  Load 200 µl of washing bufferm, wait 5 minutes, then add the remaining 200 µl and wait 1 hour.
3.  If you plan to storage flowcell for later use - add storage buffer. Otherwise load new library.
